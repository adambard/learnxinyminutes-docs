...
int main() {
    ...
    printf("%d\n", x_hex); // => Выводит 1
    printf("%d\n", (short) x_hex); // => Выводит 1
    printf("%d\n", (char) x_hex); // => выводит 1
    ...
    printf("%f\n", (float)100); // %f форматирует float
    printf("%lf\n", (double)100); // %lf форматирует double
    printf("%d\n", (char)100.0);
...
/*
Данные в функцию передаются "по значению", грубо говоря,
При вызове функций передаются копии исходных аргументов (кроме массивов).
То есть любые изменения, вносимые в аргументы внутри функции, не влияют на
значения исходных данных. Но никто не мешает вам передавать 
В функцию указатели и менять данные по указателям.
...
//Вы можете вызывать функцию в другой функции:
...
//Учтите, что объявление функции должно быть раньше её вызова.
