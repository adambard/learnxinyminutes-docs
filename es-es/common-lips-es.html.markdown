...
Common Lisp es un lenguage de proposito general y multiparadigma adecuado para una amplia variedad
...
;;; Tipicamente, S-expressions agrupadas son llamadas `forms`.

10            ; un atom; se evalua a si mismo
:thing        ; otro atom; evaluando el simbolo :thing
...
;;; commentarios de una sola linea empiezan con punto y coman; usa cuatro para
;;; commentarios a nivel de archivo, tres para descripciones de sesions, dos
;;; adentro de definiciones, y una para lineas simples. Por ejemplo,

;;;; life.lisp

;;; Foo bar baz, poruqe quu quux. Optimizado para maximo krakaboom y umph.
;;; Requerido por la funcion LINULUKO.

(defun sentido (vida)
  "Returna el sentido de la vida calculado"
  (let ((meh "abc"))
    ;; llama krakaboom
    (loop :for x :across meh
       :collect x)))                    ; guarda valores en x, luego lo retorna

;;; Comentarios de bloques, por otro lado, permite comentarios de forma libre. estos son
;;; delimitados con #| y |#

#| Este es un comentario de bloque el cual
   puede abarcar multiples lineas y
    #|
...
;;; Existe una variedad de implementaciones; La mayoria son conforme a los estandares. SBCL
;;; es un buen punto de inicio. librerias de terceros pueden instalarse facilmente con
;;; Quicklisp

;;; CL es usualmente desarrollado y un bucle de Lectura-Evaluación-Impresión (REPL), corriendo
;;; al mismo tiempo. El REPL permite la exploración interactiva de el programa mientras este esta
;;; corriendo
...
'foo ; => FOO  Note que el simbolo es pasado a mayusculas automaticamente.

;;; INTERN manualmente crea un simbolo a partir de una cadena.
...
;;; si deseas crear datos literales use QUOTE para prevenit que estos sean evaluados
...
;;; Cadenas son arrays de caracteres de longitud fija

"Hello, world!"
"Benjamin \"Bugsy\" Siegel"   ; el slash invertido es un caracter de escape
...
;;; FORMAT es usado para crear salidas formateadas, va desde simple interpolacion de cadenas
;;; hasta ciclos y condicionales. El primer argumento de FORMAT determina donde ira la cadena
;;; formateada. Si este es NIL, FORMAT simplemente returna la cadena formateada como un valor;
;;; si es T, FORMAT imprime a la salida estandar, usualmente la pantalla, luego este retorna NIL.
...
;;; Puedes crear una variable global (ambito dinamico) usando DEFVAR y DEFPARAMETER
;;; el nombre de la variable puede usar cualquier caracter excepto: ()",'`;#|\

;;; La diferencia entre DEFVAR y DEFPARAMETER es que reevaluando una expresión
;;; DEFVAR no cambia el valor de la variable. DEFPARAMETER, por otro lado si lo hace.

;;; Por convención, variables de ambito dinamico tienen "earmuffs" en sus nombres.

(defparameter *some-var* 5)
*some-var* ; => 5

;;; Puedes usar tambien caracteres unicode.
...
;;; puedes crear enlaces locales con LET. en el siguiente codigo, `me` es asignado 
...
;;; DOG-P, MAKE-DOG, y DOG-NAME son creados automaticamente por DEFSTRUCT
...
;;; CONS retorna un nuevo par CONS con el primer argumento como el primer item y el
;;; segundo argumento como el resto de el par CONS
...
;;; Vectores literales son array de longitud fija
...
;;; vectores y cadenas son casos especiales de arrays.
...
;;; Precaución: el valor inicial por defect de MAKE-ARRAY son implementaciones definidas
;;; para definirlas explicitamente:
...
;;; Este valor es definido por implementacion:
;;; NIL en ECL, 0 en SBCL and CCL.

;;; vectores ajustables

;;; los vectores ajustables tienen la misma representacion en la impresion como los vectores literales
;;; de longitud fija.
...
;;; Sin embargo, necesitaras una mejor estructura de datos que listas enlazadas
...
;;; las expresiones en CL tienen la habilidda te retornar multiples valores.
...
;;; GETHASH es una ejemplo de una funcion que retorna multiples valores. El primer
;;; valor es el valor de la llave en la tabla hash: si la llave no existe retorna NIL.

;;; El segundo valor detemina si la llave existe en la tabla hash. si la llave no existe
...
;;; Puedes declara un valor por defecto para los llaves no existentes

(gethash 'd *m* :not-found) ; => :NOT-FOUND

;;; Vamos a manejar los multiples valores de retornno en el codigo.
...
;;; Use LAMBDA para crear funciones anonimas. las funciones siempre retornan el valor 
;;; de la ultima expresion. la representacion imprimible de una funcion varia entre
;;; implementaciones.

(lambda () "Hello World") ; => #<FUNCTION (LAMBDA ()) {1004E7818B}>

;;; Use FUNCALL para llamar funciones anonimas.

(funcall (lambda () "Hello World"))   ; => "Hello World"
(funcall #'+ 1 2 3)                   ; => 6

;;; Un llamado a FUNCALL es tambien realizado cuando la expresion lambda es el CAR de
;;; una lista.
...
;;; Los () en la definicion anterior es la lista de argumentos
...
;;; Las funciones tambien tienen argumentos claves para permitir argumentos no positionados
...
;;; CL tiene un sofisticado sistema de igualdad. Una parte es tratada aqui.

;;; Para numeros use `='
(= 3 3.0)               ; => T
(= 2 1)                 ; => NIL

;;; Para identidad de objectos (aproximadamente) use EQL
(eql 3 3)               ; => T
(eql 3 3.0)             ; => NIL
(eql (list 3) (list 3)) ; => NIL

;;; para listas, cadenas, and bit-vectors use EQUAL
...
;;; COND encadena una serie de pruebas para seleccionar un resultado
...
;;; No mas clases de animales, tengamos transportes mecanicos impulsados por el humano
...
;;; 3. slot list
;;; 4. especificadoes opcionales

;;; cuando no hay lista de superclase, la lista vacia indica clase de 
;;; objeto estandar, esto puede ser cambiado, pero no mientras no sepas
...
;;; para definir un metodo, encontremos la circunferencia de la rueda usando
;;; la ecuación  C = d * pi

(defmethod circumference ((object bicycle))
  (* pi (wheel-size object)))

;;; PI es definido internamiente en CL
...
;;; un ciclo WHILE, por lo tanto es facil escribirlo, Si obedecemos nuestros
...
;;; revisemos la version de alto nivel para esto:
...
el cual puede ser movido facilmente a Common Lisp
