...
Nearly all examples below can be a part of a shell script or executed directly in the shell. A basic knowledge of common Unix/Linux commands and 
regular expressions would be very helpful in learning bash.
...
# Be aware that single quotes (') will always parse arguments literally:
echo '$VARIABLE'
# The output will literally be $VARIABLE as is.
# Double quotes, on the other hand, will recognize special characters such as
...
# inputs to ensure that the schell script program works as intended.
# The test command syntax is simply [ -options file ]. The spaces between [ ] are mandatory.
...
# This simply checks if a given variable is a file (-f) by recieving the exit status of
# the test command, and informs the user appropiately.
