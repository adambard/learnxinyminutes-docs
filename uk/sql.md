...
Реалізації зазвичай мають командний рядок, де можна інтерактивно користуватися командами наведеними нижче. Вони також пропонують спосіб виконання послідовності таких команд, збережених у вибраному скрипті. (Гарною, проте не стандартизованою, практикою буде вказувати завершення своєї роботи з командним рядком ключовими словами, що підтримуються більшістю SQL імплементацій: QUIT, EXIT, або ж обома.)
...
-- Вибрати всі рядки та колонки з таблиці departments обраної БД.
-- За змовчуванням, інтерпретатор прокручує результати на екрані.
SELECT * FROM departments;

-- Отримати всі рядки тільки колонок dept_no та dept_name з таблиці departments.
-- Це ок розділяти команди на декілька рядків.
SELECT dept_no,
       dept_name FROM departments;

-- Вибрати всі колонки departments, але обмежитись тільки 5 рядками.
SELECT * FROM departments LIMIT 5;

-- Отримати ті значення колонки dept_name з таблиці departments,
-- що містять підрядок 'en' всередині себе.
SELECT dept_name FROM departments WHERE dept_name LIKE '%en%';

-- Отримати всі стовпці з таблиці departments для тих рядків, що в колонці dept_name
...
-- Оператори порівняння (=, >, <, >=, <=, <>) та ключові слова 
-- умов (AND, OR) можна використовувати для уточнення запитів.
