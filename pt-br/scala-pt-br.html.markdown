...
lang: pt-pt
    - ["Felipe Nogueira de Souza", "http://fnsouza.me"]
lang: pt-br
---
...
/*
  ...
  1) Baixe o Scala - http://www.scala-lang.org/downloads
  2) Extraia no local em que preferir e coloque o subdiretório `bin` na
     variável de ambiente `PATH`
  3) Inicie a REPL de Scala com o comando `scala`. Você verá algo como:
  ...
  Vamos mostrar arquivos de Scala mais à frente neste tutorial mas, por enquanto,
...
/*
  Comentários de múltiplas linhas, como neste exemplo, são assim.
*/
...
// A imutabilidade é uma propriedade geralmente vantajosa.
...
/*
  ...
  inferir qual o tipo de uma variável, portanto não temos que declarar o tipo sempre.
...
/* A linha acima resulta em:
  ...
  Isto significa que o resultado de avaliar 1 + 7 é um objeto do tipo Int com
  o valor 8.

  Note que "res29" é um nome de uma variável gerado sequencialmente para
...
// 'Strings entre aspas simples não existem' <= Isto causa um erro
...
// Além disso, possuem métodos de Scala.
...
//
// Se você vem de linguagens mais tradicionais repare na omissão da palavra
// return. Em Scala, a ultima expressão de um bloco é o seu
// valor de retorno
...
// Na maior parte dos casos (sendo funções recursivas a principal exceção), o
...
// Funções anonimas são definidas da seguinte forma:
(x: Int) => x * x

// Ao contrário de defs, o tipo de entrada de funções anonimas pode ser omitido
...
// Funcões anonimas são chamadas como funções normais:
quadrado(10)   // => 100

// Se cada argumento de uma função anonima for usado apenas uma vez, existe
// uma forma ainda mais curta de os definir. Estas funções anonimas são
// extremamente comuns, como será visto na seção sobre estruturas de dados.
...
// Não tem qualquer efeito em funções anonimas, por exemplo:
def foo(x: Int): Int = {
...
r foreach println
// NB: Scala é bastante suave em relação a pontos e parenteses - estude as
...
// A forma idiomática em Scala de definir ações recorrentes é através de
// recursão em cauda.
 ...
 * e verifique que a consegue ler
 */
...
divideInts(10, 3) // A função divideInts retorna o resultado e o resto

// Para acessar aos elementos de uma tupla, pode-se usar _._n, onde n é o indice
...
/*
  Observação: Até agora tudo o que fizemos neste tutorial foram expressões simples
  ...
  isoladas num arquivo de Scala. Por exemplo, não é possivel rodar um
  arquivo scala que apenas contenha "val x = 5". Em vez disso, as únicas
...
// Classes são semelhantes a classes em outras linguagens. Os argumentos do
...
class Cão(rc: String) {
  ...
  // Métodos abstratos são métodos sem corpo. Se descomentarmos a próxima
...
// linguagens. Note que objetos e classes podem ter o mesmo nome.
object Cão {
...
// case classes. A linha é vaga mas, em geral, classes tendem a concentrar-se
...
// métodos, sendo que estes raramente possuem efeitos secundários.
...
// Case classes trazem algumas vantagens de graça, como getters:
jorge.telefone  // => "1234"
...
// graça, como pode ser visto de seguida.
...
// Na realidade, é possível fazer correspondência com qualquer objeto que
...
// código "necessite" de um valor implicito. Um exemplo são argumentos
...
// Neste caso, a String é primeiro convertida para Cão usando a nossa função,
...
// ao definir a função implicita, o compilador deve exibir um aviso 
// que só deve definir a função se souber o que está fazendo.
...
// O ponto de entrada de um programa em Scala é definido por um arquivo .scala
// com um método main:
object Aplicação {
...
// Ficheiros podem conter várias classes o objetos. Compilar com scalac
