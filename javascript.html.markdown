...
a lot of the basic syntax will already be familiar. Despite this, and
the similarity in name, JavaScript's object model is significantly different
from Java's.
...
// Join all elements of an array with a semicolon, or with a semicolon and
// a space
...
// The for/of statement allows iteration over iterable objects (including the
// built-in String,
// Array, e.g. the Array-like arguments or NodeList objects, TypedArray, Map
// and Set, and user-defined iterables).
var myPets = "";
...
// made available to the function via the `this` keyword. Functions designed to
// be called like that are called constructors.
...
// We mentioned that `__proto__` was non-standard, and there's no standard way
// to change the prototype of an existing object. However, there are two ways to
...
// However, the wrapper objects and the regular built-ins share a prototype, so
...
// The "equivalent" of this function in the traditional syntax would look like:
...
can help others out by sharing your knowledge.

MDN's [A re-introduction to JavaScript][2] covers many of the concepts covered
...
JavaScript on web pages, start by learning about the [Document Object Model][3].

[Learn Javascript by Example and with Challenges][4] is a variant of this
reference with built-in challenges.

[JavaScript Garden][5] is an in-depth guide to all the counter-intuitive parts
of the language.
