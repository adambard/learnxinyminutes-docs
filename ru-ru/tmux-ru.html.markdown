...
Сессия tmux также может быть закрыта из экрана терминала, и она
...
```
  ...
  tmux [command]     # Запуск команды 'tmux'
    ...
    ls               # Список открытых сессий
     -a              # Список всех открытых сессиий
...
----------------------------------------------------------------------
  ...
  Up, Down           # Переместиться на панель выше, ниже, левее
  Left, Right        # или правее
...
tmux.conf файл может быть использован для автоматической установки
...
# Активировать inner-most сессию (когда вложенный tmux) для отправки команд
bind a send-prefix
...
# Цветовая палитра окна статусов
