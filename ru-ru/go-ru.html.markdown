...
import (
    ...
    "io/ioutil" // io - низкоуровневые функции ввода/вывода (input/output),
    // ioutil - дополнительные вспомогательные высокоуровневые функции.
    m "math"   // стандартная библиотека math под локальным именем m.
    "net/http" // net - функции  и типы для организации обмена данными через сетевые сокеты по TCP/IP и UDP.
    // net/http - обеспечивает возможность парсинга HTTP-запросов и ответов, а также реализует HTTP-клиент.
...
func beyondHello() { // Функция приветствия.
    ...
    x = 3     // Присваивание значениz переменной.
    // Синтаксически ":=" - это определение типа, объявление и присваивание переменной одновременно.
    y := 4
...
может включать разрывы` // Тоже string, причем `я` позволяет переносить строки.
    ...
    g := 'Σ' //  Тип - rune, синоним типа int32 при работе с отдельными символамим, стандарта Юникод.
...
.
    ...
    var u uint = 7 // беззнаковое целое, но использование зависит от размера, как с типом "int".
    var pi float32 = 22. / 7/
    ...
    var a4 [4]int           // Этот массив состоит из 4 значений типа int, все инициализируются 0.
    ...
    // Массив переменного объема обновляется, как в примере ниже:
    ...
    // Подчеркивания позволяют использовать переменную, но отбросить приваиваемое ей значение.
...
func learnMemory() (p, q *int) { // Функция изучения работы с памятью.
    ...
    s := make([]int, 20) // Размещается 20 значений типа int в одном блоке памяти.
    s[3] = 7             // Один из них замещается.
    r := -2              // Объявляется еще одна локальная переменная.
