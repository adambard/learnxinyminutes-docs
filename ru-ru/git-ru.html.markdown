...
Контроль версий - заключается в длительном наблюдении за объектом (файлом[и])
и записыванием всех происходящих с ним изменений.
...
Также возможно название синоним **сцена**

### Commit

Срез изменений, которые были совершены с момента предыдущего `commit`.
Их можно опубликовать в другие репозитории, а можно и нет.
...
Ссылка на определенный момент в истории проекта, например на момент релиза.
...
- Staged (промежуточное) - процесс определения изменений, который войдут
  в `commit`
...
```bash
# Покажет текущий филиал, неотслеживаемые файлы, изменения
# и другие отличия, который произошли с последнего commit
$ git status
...
# возможно если у Вас есть права на запись в сервере,
...
# Клонирование специального ответвления
