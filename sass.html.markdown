...
/* You can also use variables in selectors using the #{...} operator.
   Note: there should not be a backslash in front of the # sign, it is just
   to allow the comment to be compiled by SASS without throwing an error. */
...
/* '&' will be replaced by the parent selector and can be placed before or after other selectors. */
