...
fn main() {
    ...
    // Μεταβλητές (με την έννοια των διαδικαστικών και αντικειμενοστρεφών γλωσσών προγραμματισμού). Σε αντίθεση
    // με τις περισσότερες από αυτές, στη Rust η αμετάβλητη δέσμευση είναι στάνταρ. Το mut δηλώνει μεταβλητότητα.
    let mut mutable = 1;
    mutable = 4;
    mutable += 2;
